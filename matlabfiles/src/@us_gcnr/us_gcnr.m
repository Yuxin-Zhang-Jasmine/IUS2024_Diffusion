classdef us_gcnr < handle
    
    %-- Class defining a testing procedure to assess gCNR performance of 
    ...
    methods (Access = public)
        ...
        function evaluate(h)
            ...
            for f=frame_list            
                ...
                %-- yuxin edit
                bmode(isinf(bmode))=-10000;
                bmode(isnan(bmode))=-10000;
                if h.image.postenv == 1
                ...
                for k=1:length(h.pht.occlusionDiameter)
                    ...
                    % yuxin edit the coe from 1.2 to 1
...
%                 	tmpfig = figure(22);					
% 	                histo = histogram([inside(:);outside(:)],256);					
% 	                bEdges = histo.BinEdges;
% 	                hold on
% 	                fhisto = histogram(inside(:), bEdges, 'Normalization','probability');					
% 	                ghisto = histogram(outside(:), bEdges, 'Normalization','probability');
% 	                fv = fhisto.Values;					
% 	                fv = fv/sum(fv(:));
% 	                gv = ghisto.Values;
% 	                gv = gv/sum(gv(:));
...
% 	                close(tmpfig)
                    ...
                    bins = 256;
                    [~, edges] = histcounts([inside(:);outside(:)], bins);
                    
                    fv = histcounts(inside(:), edges, 'Normalization', 'probability');
                    gv = histcounts(outside(:), edges, 'Normalization', 'probability');
                    
                    fv = fv / sum(fv);
                    gv = gv / sum(gv);
                    
                    h.score(f,k) = 1 - sum(min(fv, gv));
