function exec_evaluation_contrast_speckle(path_scan,path_phantom,path_img,flag_simu,flag_display,path_output)
    ...
    testing_gcnr = us_gcnr();
    testing_gcnr.pht = pht;
    testing_gcnr.scan = scan;
    testing_gcnr.image = image;
    testing_gcnr.flagDisplay = flag_display;
    testing_gcnr.evaluate();
    ...
    %-- Perform testing for speckle quality
    testing_snr = us_speckle_SNR();
    testing_snr.pht = pht;
    testing_snr.scan = scan;
    testing_snr.image = image;
    testing_snr.flagDisplay = flag_display;
    testing_snr.evaluate();    
    ...
    for c=1:length(category)
        ...
        if ( indexCategory(c)~=0 )
            ...
            score_gcnr = squeeze(testing_gcnr.score(idx,:,:));
            writeScoreGCNR(fid,score_gcnr,flag_simu);
            score_speckle = squeeze(testing_speckle.score(idx,:));
            writeScoreSpeckle(fid,score_speckle);
            score_snr = squeeze(testing_snr.score(idx,:));
            writeScoreSNR(fid,score_snr);
        end            
        fprintf(fid,'\n#END\n\n');            
    end    
    ...
    if (pw_bonus == 0)            
        ...
        score_gcnr = squeeze(testing_gcnr.score(idx(1),:,:));
        writeScoreGCNR(fid,score_gcnr,flag_simu);
        score_speckle = squeeze(testing_speckle.score(idx(1),:));
        writeScoreSpeckle(fid,score_speckle);   
        score_snr = squeeze(testing_snr.score(idx(1),:));
        writeScoreSNR(fid,score_snr);
    end
    fprintf(fid,'\n#END\n\n');
...
%-- Write gCNR results 
function writeScoreGCNR(fid,score,flag_simu)

    if (flag_simu==1)
        writeSimulationScoreGCNR(fid,score);
    else
        writeExperimentScoreGCNR(fid,score);
    end
    
end


%-- Write gCNR results corresponding to numerical phantom
function writeSimulationScoreGCNR(fid,score)
        
    %-- Mean axial and lateral values
    fprintf(fid,'------------------------------------\n');
    fprintf(fid,'Mean gCNR scores: %.2f \n\n',mean(score));
...
%-- Write gCNR results corresponding to ex-vivo phantom
function writeExperimentScoreGCNR(fid,score)
...
%----------------------------------------------------------------
%-- Write SNR results 
function writeScoreSNR(fid,score)

    %-- Speckle score display (0: test failed || 1: test passed)
    fprintf(fid,'------------------------------------\n');    
    fprintf(fid,'Mean SNR scores: %.2f \n\n',mean(score)); 
