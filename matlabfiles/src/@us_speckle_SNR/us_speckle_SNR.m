classdef us_speckle_SNR < handle
    ...
    methods (Access = public)
        ...
        function h = us_speckle_SNR(input_name)
            if exist('input_name') 
                h.name= input_name; 
            else
                h.name = ' ';
...
%                     h.score(f,k) = testres;
                    if (h.flagDisplay==1)
                        figure(1);
                        hold on; contour(h.scan.x_axis*1e3,h.scan.z_axis*1e3,maskROI,[1 1],'b-','linewidth',2);
                        pause(0.1);
                    end
                    h.score(f,k) = mean(sample)/std(sample);
