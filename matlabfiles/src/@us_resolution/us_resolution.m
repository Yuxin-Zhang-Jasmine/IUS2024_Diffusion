    ...
    methods (Access = public)
        ...
        function evaluate(h)
            ...
            for f=frame_list
                ...
                %-- yuin edit
                bmode(isnan(bmode)) = -1e4;
                bmode(isinf(bmode)) = -1e4;
                if h.image.postenv == 1
                    [bmode,~] = envelope(bmode,1,'peak');
                elseif h.image.postenv == 2
                    [bmodet,~] = envelope(bmode',1,'peak');
                    bmode = bmodet';
                end
...
%                     %-- (yuxin) in case ther are more than one maxs  (happens in synthetic images)
% %                     if length(idz)>1
% %                         idz = idz(1);
% %                         idx = idx(1);
% %                     end
%                     if length(idz)>1
%                         tmp = ceil(length(idx)/2)+1;
%                         idz = idz(tmp);
%                         idx = idx(tmp);
%                     end
                    %---------------------------------------------
    ...
    methods (Access = private)
        ...
        function [res] = Compute_6dB_Resolution(h,x_axis,y_signal,num,color)   
            ...
            ind = find(y_interp >= (max(y_interp)-15) ); % yuxin edit 6 to 30,40,45 here
            idx1 = min(ind);
            idx2 = max(ind);
